...
class NVDM(object):
    def __init__(self,
        ...
        with tf.variable_scope('encoder'):
            ...
            self.enc_vec = utils.mlp(self.enc_input, mlp_arr, self.non_linearity)
            ...
            self.phi = tf.contrib.layers.batch_norm(utils.linear(self.clss_mlp, n_class, scope='phi'),is_training=self.training) #y logits
    ...
    def run_model(self,labeled_training_rounds,
        ...
        for x in range(int(labeled_training_rounds)):
            for idx_batch in train_batches_with_lab:
                ...
                data_batch_y = utils.fetch_data_y(train_set_y, idx_batch, self.n_class)
                input_feed = {self.x.name: data_batch, self.mask.name: mask, self.keep_prob.name: keep_prop,self.prob:dropout,
                              self.warm_up.name: warm_up, self.min_alpha.name: min_alpha,
                              self.lab: np.ones((1)), self.y.name: data_batch_y,self.idx.name:np.zeros((1),dtype=np.int32),
                              self.training.name:training}
                if optim:
        ...
        if semi_supervised and len(train_batches_without_lab)>0:
            for clss_idx in range(self.n_class):
                for idx_batch in train_batches_without_lab:
                    ...
                    data_batch_y = utils.fetch_data_y_dummy(idx_batch, self.n_class,clss_idx)
                    input_feed = {self.x.name: data_batch, self.y.name: data_batch_y, self.mask.name: mask,
                                  self.keep_prob.name: keep_prop, self.lab: np.zeros((1)),self.prob:dropout,
                                  self.warm_up.name: warm_up, self.min_alpha.name: min_alpha,
    ...
    def train_x(self,dev_set_with_lab,
              dev_set_without_lab,
              dev_set_y,
              train_set_with_lab,
